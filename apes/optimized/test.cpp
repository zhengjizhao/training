	...
	*/
    ...
    //    cvt2.V0zI[0] -=3e6/N_turns;
	    ...
	    for (unsigned int j = 0;j< N_bnches_p_train*N_trns;++j){
	        ...
	        //ZZstart = std::chrono::high_resolution_clock::now();
	        bm1.bnches[j].sort();
	        //ZZfinish = std::chrono::high_resolution_clock::now();
	        //ZZelapsed = finish - start;
	        //ZZstd::cout<<"Time spent on sorting: "<< elapsed.count()*1000<<" ms."<<std::endl;
	        
    //	    std::cout<<"Calculating wake..."<<std::endl;
            //ZZstart = std::chrono::high_resolution_clock::now();
    //        cvt.wake_Naive(bm1,bm1.bnches[j]);
    //        cvt.wake_Naive_parasec(bm1,bm1.bnches[j]);
            cvt.wake_Naive_base(bm1,bm1.bnches[j]);
            //ZZfinish = std::chrono::high_resolution_clock::now();
            //ZZelapsed = finish - start;
	        //ZZstd::cout<<"Time spent on waking: "<< elapsed.count()*1000<<" ms."<<std::endl;
		   
		   //ZZ start = std::chrono::high_resolution_clock::now();
	//	    cvt.update_coord_no_wake_1D(bm1.bnches[j]);
	//	    cvt.update_coord_no_wake(bm1.bnches[j]);
            cvt.update_coord(bm1.bnches[j]);
            //ZZfinish = std::chrono::high_resolution_clock::now();
            //ZZelapsed = finish - start;
	        //ZZstd::cout<<"Time spent on kicking: "<< elapsed.count()*1000<<" ms."<<std::endl;
	...
	//	    cvt2.update_coord(bm1.bnches[j]);
            ...
            //ZZfinish = std::chrono::high_resolution_clock::now();
            //ZZelapsed = finish - start;
	        //ZZstd::cout<<"Time spent on going around the ring: "<< elapsed.count()*1000<<" ms."<<std::endl;
	...
	//	    temp_t.push_back(bm1.bnches[0].t[0]*2*pi*cvt.frq[0]);
            ...
            //ZZfinish = std::chrono::high_resolution_clock::now();
            //ZZelapsed = finish - start;
	        //ZZstd::cout<<"Time spent on update bunch moments: "<< elapsed.count()*1000<<" ms."<<std::endl;
            ...
            //ZZfinish = std::chrono::high_resolution_clock::now();
            //ZZelapsed = finish - start;
	        //ZZstd::cout<<"Time spent on recording the results in buffer: "<< elapsed.count()*1000<<" ms."<<std::endl;
	//	    std::cout<<std::endl;
