...
public:
    ...
    std::vector<unsigned int> slices; // sliced bunch, each element holds the number of macro particles fall in that bin.
	...
	bunch(unsigned int disType,
		...
		unsigned int Npar,double t0, unsigned int N_bins) {
		Np = Npar;
...
/*
        std::sort(std::execution::par,x.begin(),x.end(),[&](const double& a,const double& b){return (t[a]<t[b]);});
       std::sort(std::execution::par,px.begin(),px.end(),[&](const double& a,const double& b){return (t[a]<t[b]);});
        std::sort(std::execution::par,y.begin(),y.end(),[&](const double& a,const double& b){return (t[a]<t[b]);});
       std::sort(std::execution::par,py.begin(),py.end(),[&](const double& a,const double& b){return (t[a]<t[b]);});
        std::sort(std::execution::par,pz.begin(),pz.end(),[&](const double& a,const double& b){return (t[a]<t[b]);});
...
*/
        ...
        sort_based_on_index(x,index);
        sort_based_on_index(y,index);
        sort_based_on_index(px,index);
        sort_based_on_index(py,index);
        sort_based_on_index(pz,index);
        __gnu_parallel::sort(t.begin(),t.end());
...
public:
	...
	// Defualt initializer.
	beam(unsigned int N,unsigned int N_bnches_per_train, unsigned int N_trains,double dly, double gp, unsigned int N_bins) {
	...
	beam(
		...
		std::vector<double> spcSig, // vector that contain sigma of distribution in spacial coordinates
		std::vector<double> mmtSig, // vector that contain sigma of distribution in momentum coordinates
		unsigned int N, unsigned int N_bnches_per_train,unsigned int N_trains, unsigned int N_bins) // particles per bunch and bunches per beam.
		{
        bnches.resize(N_bnches_per_train*N_trains);
		delay = dly;
		gap = gp;
		for (int i = 0;i<N_trains;++i){
            for (int j = 0;j<N_bnches_per_train;++j){
		        bnches[i] = bunch(N, spc0, mmt0, spcSig, mmtSig, N,i*gap+(i*N_bnches_per_train+j)*delay, N_bins);
		    }
		}
	};
};
...
public:
	...
	void wake_Naive(beam& bm, bunch& bnch){
	    ...
	    double qoc = bnch.qe/c;// charge over speed of light.
        int j = 0;
#pragma omp parallel for private(j)
	    for (int i = 0;i<N_mod;++i){ // iterate over number of modes.
	        ...
	        // kick the first particle
            bnch.px[0] += qoc*(V0xR[i]*cosphi-V0xI[i]*sinphi+VxTauR[i][0]); // kick in x direction. cannot see self field.
			bnch.py[0] += qoc*(V0yR[i]*cosphi-V0yI[i]*sinphi+VyTauR[i][0]); // kick in y direction.
			bnch.pz[0] += qoc*((V0zR[i]*cosphi-V0zI[i]*sinphi+VzTauR[i][0])+(V0xR[i]*cosphi-V0xI[i]*sinphi)*(2*pi*fi)/c*bnch.x[0]+(V0yR[i]*cosphi-V0yI[i]*sinphi)*(2*pi*fi)/c*bnch.y[0]+Vbz*0.5); // kick in z direction
			// kick the rest of the particles.
			unsigned int Block_size = 64; // try to run the sine cosine in block

	        for (j = 1;j<bnch.Np;j+=Block_size){ // iterate over every particles. 
	            ...
	            //double dt = bnch.t[j]-bnch.t[j-1];
	            
                std::vector<double> vcos(Block_size,0);
                std::vector<double> vsin(Block_size,0);
                std::vector<double> vcosphi(Block_size,0);
                std::vector<double> vsinphi(Block_size,0);

#pragma omp simd                
	            for (unsigned int k = 0;k<Block_size;++k){
	                vcos[k] = cos(2.0*pi*frq[i]*bnch.dt[j+k]);
	                vsin[k] = sin(2.0*pi*frq[i]*bnch.dt[j+k]);
	                vcosphi[k] = cos(2.0 * pi*fi*bnch.t[j+k]+phiN);
	                vsinphi[k] = sin(2.0 * pi*fi*bnch.t[j+k]+phiN);
	            }
#pragma unroll(8)	            
			    for(unsigned int k = 0;k<Block_size;++k){
	                VxTauR[i][j+k] = VxTauR[i][j+k-1]*vcos[k]-(VxTauI[i][j+k-1]+Vbx*bnch.x[j+k])*vsin[k];
	                VyTauR[i][j+k] = VyTauR[i][j+k-1]*vcos[k]-(VyTauI[i][j+k-1]+Vby*bnch.y[j+k])*vsin[k];
	                VzTauR[i][j+k] = (VzTauR[i][j+k-1]+Vbz)*vcos[k]-VzTauI[i][j+k-1]*vsin[k];
	                VxTauI[i][j+k] = VxTauR[i][j+k-1]*vsin[k]+(VxTauI[i][j+k-1]+Vbx*bnch.x[j+k])*vcos[k];
	                VyTauI[i][j+k] = VyTauR[i][j+k-1]*vsin[k]+(VyTauI[i][j+k-1]+Vby*bnch.y[j+k])*vcos[k];
	                VzTauI[i][j+k] = (VzTauR[i][j+k-1]+Vbz)*vsin[k]+VzTauI[i][j+k-1]*vcos[k];
	                
	                bnch.px[j+k] += qoc*(V0xR[i]*vcosphi[k]-V0xI[i]*vsinphi[k]+VxTauR[i][j+k]); // kick in x direction. cannot see self field.
			        bnch.py[j+k] += qoc*(V0yR[i]*vcosphi[k]-V0yI[i]*vsinphi[k]+VyTauR[i][j+k]); // kick in y direction.
			        bnch.pz[j+k] += qoc*((V0zR[i]*vcosphi[k]-V0zI[i]*vsinphi[k]+VzTauR[i][j+k])+(V0xR[i]*vcosphi[k]-V0xI[i]*vsinphi[k])*(2*pi*fi)/c*bnch.x[j+k]+(V0yR[i]*vcosphi[k]-V0yI[i]*vsinphi[k])*(2*pi*fi)/c*bnch.y[j+k]+Vbz*0.5); // kick in z direction
			    }
	        }
	...
	void wake_Naive_parasec(beam& bm, bunch& bnch){
	    if (wake_initialized == 0){
	        for (int i = 0;i<N_mod;++i){
	            VxTauR[i].resize(bnch.Np+1,0); // extra one element at the end, use to store the wake in this mode when bunch leaves the cavity.
	            VyTauR[i].resize(bnch.Np+1,0);
...
#pragma omp parallel for private(j) num_threads(64) 
	    for (int i = 0;i<N_mod;++i){ // iterate over number of modes.
			...
			// kick the rest of the particles.
			std::vector<double> cosin(bnch.Np);
			std::vector<double> sine(bnch.Np);
			unsigned int Block_size = 32; // try to run the sine cosine in block

                #pragma unroll(8)
                for (j = 1;j<bnch.Np;++j){ 
	            cosin[j] = cos(2.0*pi*frq[i]*bnch.dt[j]);
	            sine[j] = sin(2.0*pi*frq[i]*bnch.dt[j]);
	        }
	        ...
	        for (j = 1;j<bnch.Np;++j){ // iterate over every particles. 
	            ...
	            VzTauR[i][j] = (VzTauR[i][j-1]+Vbz)*cosin[j]-VzTauI[i][j-1]*sine[j];
	            VyTauR[i][j] = VyTauR[i][j-1]*cosin[j]-(VyTauI[i][j-1]+Vby*bnch.y[j])*sine[j];
	            VxTauR[i][j] = VxTauR[i][j-1]*cosin[j]-(VxTauI[i][j-1]+Vbx*bnch.x[j])*sine[j];
	            //VxTauI[i][j] = VxTauR[i][j-1]*sine[j]+(VxTauI[i][j-1]+Vbx*bnch.x[j])*cosin[j];
	            //VyTauI[i][j] = VyTauR[i][j-1]*sine[j]+(VyTauI[i][j-1]+Vby*bnch.y[j])*cosin[j];
	            //VzTauI[i][j] = (VzTauR[i][j-1]+Vbz)*sine[j]+VzTauI[i][j-1]*cosin[j];			    
	        }
	...
	void wake_Naive_base(beam& bm, bunch& bnch){
	    if (wake_initialized == 0){
#pragma omp parallel for 
	        for (int i = 0;i<N_mod;++i){
	            VxTauR[i].resize(bnch.Np+1,0); // extra one element at the end, use to store the wake in this mode when bunch leaves the cavity.
...
#pragma omp parallel for private(j)
		...
		std::vector<double> cosin(bnch.Np,0.0);
		std::vector<double> sine(bnch.Np,0.0);
#pragma omp simd
		for(unsigned k = 0;k<bnch.Np;++k){

	            double dphi = 2.0*pi*frq[i]*bnch.dt[k];
	            cosin[k] = cos(dphi);
	            sine[k] = sin(dphi);
		}
		// kick the rest of the particles.
#pragma unroll(8)
	        for (j = 1;j<bnch.Np;++j){ // iterate over every particles. 
	            ...
	            /*
	            double dphi = 2.0*pi*frq[i]*bnch.dt[j];
	            double cosin = cos(dphi);
	            double sine = sin(dphi);
	            */
		/*    
...
#pragma omp parallel for 
		    for (int j = 0; j<bnch.Np; ++j) { // iterate over all particles
			    ...
			    bnch.px[j] += qoc*(V0xR[i]*cosphi-V0xI[i]*sinphi+VxTauR[i][j]); // kick in x direction. cannot see self field.
			    bnch.py[j] += qoc*(V0yR[i]*cosphi-V0yI[i]*sinphi+VyTauR[i][j]); // kick in y direction.
			    bnch.pz[j] += qoc*((V0zR[i]*cosphi-V0zI[i]*sinphi+VzTauR[i][j])+(V0xR[i]*cosphi-V0xI[i]*sinphi)*(2*pi*fi)/c*bnch.x[j]+(V0yR[i]*cosphi-V0yI[i]*sinphi)*(2*pi*fi)/c*bnch.y[j]+Vbz*0.5); // kick in z direction
		    }
		}
		// Update the bunch momentum and energy.
		bnch.p0 = std::accumulate(bnch.pz.begin(),bnch.pz.end(),0.0)/bnch.pz.size();
